...
public class Investing extends Operator {
  ...
  public void rename() {
    ...
    for (File f : files) {
      ...
      for (Entry<String, String> entity : this.source.entrySet()) {
        ...
        if (!match.matches()) {
          this.error(new Exception(String.format("unknown file name regex: %s", f.getName())));
          continue;
        }
        ...
        if (fileName.equals(base)) {
          ...
          final String dstPath = Paths.get(this.dstDir, "investing.com", String.format("%s.csv", symbol)).toString();
  ...
  public void check() {
    this.source.forEach((page, symbol) -> {
      final String filePath = Paths.get(this.dstDir, "investing.com", String.format("%s.csv", symbol)).toString();
