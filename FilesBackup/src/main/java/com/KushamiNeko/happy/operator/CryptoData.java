...
public class CryptoData extends Operator {
  ...
  public void rename() {
    ...
    for (File f : files) {
      ...
      if (match.find()) {
        ...
        final String dstPath = Paths.get(this.dstDir, "cryptodata", String.format("%s.csv", symbol)).toString();
  ...
  public void check() {
    ...
    this.source.forEach((url) -> {
      ...
      if (match.find()) {
        final String symbol = match.group(1).toLowerCase();
        final String filePath = Paths.get(this.dstDir, "cryptodata", String.format("%s.csv", symbol)).toString();
